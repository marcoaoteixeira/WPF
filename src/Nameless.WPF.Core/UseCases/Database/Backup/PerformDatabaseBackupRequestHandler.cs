...
public class PerformDatabaseBackupRequestHandler : IRequestHandler<PerformDatabaseBackupRequest, PerformDatabaseBackupResponse> {
    ...
    public async Task<PerformDatabaseBackupResponse> HandleAsync(PerformDatabaseBackupRequest request, CancellationToken cancellationToken) {
        ...
        var backupRelativeFilePath = databaseBackupResult.AsResult ??
                                     throw new InvalidOperationException(Exceptions.PerformDatabaseBackupRequestHandler_MissingBackupResultFilePath);
    ...
    private async Task<Result<string>> ExecuteDatabaseBackupAsync(CancellationToken cancellationToken) {
        ...
        try {
            await _notificationService.PublishAsync(PerformDatabaseBackupNotification.BackupDatabaseStarting())
                                      .SuppressContext();
            ...
            await _notificationService.PublishAsync(PerformDatabaseBackupNotification.BackupDatabaseFinish())
                                      .SuppressContext();
            ...
            var failure = PerformDatabaseBackupNotification.BackupDatabaseFailure(ex.Message);
